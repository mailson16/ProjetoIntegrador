...
if ($btn =='1') {
	...
	$sql = "select SUM(VALOR_PEDIDO) as VALOR_GERAL, count(*) as totalPedidos from pedido_vendedor 
	...
	while ($array = mysqli_fetch_array($lista_total)){
		...
		$totalPedidos = $array['totalPedidos'];
	}

	//valor total usuarios neste periodo
	$sqlCliente = " select COUNT(*) as qtd FROM cliente_fisico 
	where STATUS_CLIENTE = 'A'
	and TIPO_CLIENTE in('C','V')
	and DATA_CADASTRO BETWEEN '$data_de' and '$data_ate'";
	$lista_totalCliente = mysqli_query($conexao,$sqlCliente);
	while ($arrayCliente = mysqli_fetch_array($lista_totalCliente)){
		$totalClientes = $arrayCliente['qtd'];
	}

}
//verifica se existe anunucio a ser aprovado
...
<html>
<head>
	<title>Relatório de Geral</title>
			...
			<div class="container" id="caixa">
				...
				<div class="row" style="justify-content: center;">
					<div class="form-group col-md-1">
						<button onclick=" return verifica()" class="btn btn-outline-dark btn-md " name="pesquisa" value="1">&nbsp;&nbsp;Buscar&nbsp;&nbsp;</button>
					</div>
				</div>
			</div>
			<br>
			...
			?>
				...
				<div>
					<div>
						...
						<h4>Total de Pedidos: <?php echo $totalPedidos?></h4>
						<h4>Novos usuários: <?php echo $totalClientes?></h4>
        ...
        var data = google.visualization.arrayToDataTable([
          ...
          $sqlId = "select pr.id_produto from carrinho_compras cc 
			...
			while ($arrayId = mysqli_fetch_array($lista_id)){
				...
				and p.STATUS_PEDIDO = 'A'
				and p.DT_PEDIDO BETWEEN '$data_de 00:00:01' and '$data_ate 23:59:59'
				and p.COD_VENDEDOR = pr.cod_cliente GROUP by cc.NOME_PRODUTO";
